...
const countryNames = {
  ...
  GB: 'Reino Unido',
  PL: 'Polonia',
  PT: 'Portugal', // Añadido Portugal que aparece en tu JSON
};
...
const extractCountriesFromLocation = (location) => {
  ...
  if (locationLower.includes('españa') || locationLower.includes('peninsula')) {
    countries.push('ES'); // Añadimos código ISO de España
  }
  if (locationLower.includes('portugal')) {
    countries.push('PT'); // Añadimos código ISO de Portugal
  }
  if (locationLower.includes('francia') || locationLower.includes('france')) {
    countries.push('FR'); // Añadimos código ISO de Francia
  }
  if (locationLower.includes('italia') || locationLower.includes('italy')) {
    countries.push('IT'); // Añadimos código ISO de Italia
  }
  if (locationLower.includes('alemania') || locationLower.includes('germany')) {
    countries.push('DE'); // Añadimos código ISO de Alemania
  }
  if (locationLower.includes('reino unido') || locationLower.includes('united kingdom')) {
    countries.push('GB'); // Añadimos código ISO de Reino Unido
  }
  if (locationLower.includes('polonia') || locationLower.includes('poland')) {
    countries.push('PL'); // Añadimos código ISO de Polonia
  }
  
  return countries; // Devolvemos array con códigos ISO encontrados
};

// Datos de ejemplo por defecto en caso de que no se pasen datos externos
const defaultButterfliesData = [
  {
    id: "1",
    name: 'Swallowtail Europea',
    family: 'Papilionidae',
    Location: 'Francia, España, Italia, Alemania',
  ...
  },
  {
    ...
    name: 'Adonis Blue',
    family: 'Lycaenidae',
    Location: 'Francia, España, Italia',
    image: 'https://images.unsplash.com/photo-1581833971358-2c8b550f87b3?w=300&h=200&fit=crop',
  },
];

// Componente principal Map que recibe datos de mariposas y función callback
const Map = ({ butterfliesData = [], onButterflySelect }) => {
  ...
  // Usamos datos externos si existen, sino usamos datos por defecto
  ...
  return (
    ...
    <div className="w-full max-w-7xl mx-auto p-4">
      ...
      >
        ...
        {/* Modal que aparece cuando se selecciona un país */}
        {selectedCountry && (
          // Overlay de fondo oscuro semitransparente
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            {/* Contenedor principal del modal */}
            <div className="rounded-xl max-w-4xl w-full max-h-96 overflow-auto shadow-2xl" style={{ backgroundColor: '#F5F5DC' }}>
              ...
              <div className="p-6">
                {/* Texto descriptivo con cantidad de especies */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {selectedCountry.butterflies.map((butterfly) => (
                    ...
                    <div
                      ...
                      onClick={() => {
                        // Si hay función callback, la ejecutamos con la mariposa seleccionada
                    ...
                    >
                      ...
                      {/* Información de la mariposa debajo de la imagen */}
                      <div className="p-4">
                        {/* Nombre común de la mariposa */}
                        <h4 className="font-bold text-purple-800 mb-2 text-center" style={{ fontFamily: 'serif' }}>
                          {butterfly.name}
                        </h4>
                        
                        {/* Información adicional si está disponible */}
                        {butterfly.family && (
                          <p className="text-sm text-gray-600 text-center mb-1" style={{ fontFamily: 'serif' }}>
                            Familia: {butterfly.family}
                          </p>
                        )}
                        
                        {/* Mensaje para explorar más detalles */}
                        <p className="text-xs text-center mt-2" style={{ color: '#8B7355', fontFamily: 'serif' }}>
                          Toca para explorar →
                        </p>
                      </div>
                    </div>
